...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add'])) {
        ...
        $stmt = $db->prepare("INSERT INTO Coupons (id, code, discount, usage_limit, expire_date, company_id, created_at) VALUES (:id, :code, :discount, :limit, :exp, :comp, :created_at)");
        $stmt->execute([
            'id' => $id,
            'code' => $_POST['code'],
        ...
        $stmt = $db->prepare("UPDATE Coupons SET code = :code, discount = :discount, usage_limit = :limit, expire_date = :exp WHERE id = :id AND company_id = :comp");
        $stmt->execute([
            'code' => $_POST['code'],
            'discount' => $_POST['discount'],
            'limit' => $_POST['usage_limit'],
        ...
        $db->prepare("DELETE FROM Coupons WHERE id = :id AND company_id = :comp")->execute(['id' => $_POST['id'], 'comp' => $company_id]);
    }
}

$stmt = $db->prepare("SELECT * FROM Coupons WHERE company_id = :comp");
